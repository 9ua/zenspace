<template>
    <div class="swipe">
        <div v-if="count > 1" :style="trackStyle" class="swipe-track" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd" @touchcancel="onTouchEnd" @transitionend="$emit('change', activeIndicator)">
            <slot />
        </div>
        <div v-else class="swipe-track">
            <slot />
        </div>
        <div class="swipe-indicators" v-if="showIndicators && count > 1">
            <i v-for="index in count" :class="{ 'swipe-indicator--active': index - 1 === activeIndicator }" :key='index'/>
        </div>
    </div>
</template>
<script>
