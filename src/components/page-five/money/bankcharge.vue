	...
	<div class="listStyle">
		...
		<div class="listStyle-content">
			<ul class="listStyle-VI">
				...
				<li style="background:#ffeeee;height:42px;">
					<p>收款人姓名</p>
					<span>{{receiveNiceName}}</span>
				</li>
				<li style="background:#ffeeee;height:46px;">
					<p>收款卡号</p>
					<span style="font-size:20px;">{{receiveCard}}</span>
				</li>
				<li style="background:#ffeeee;height:42px;">
					<p>收款银行地址</p>
			...
			<div class="warning" >
        		<p>1、请转账到以上收款银行账户。</p><br>
        		<p>2、请正确填写转账银行卡的持卡人姓名和充值金额，以便及时核对。</p><br>
...
export default {
	...
	methods :{
		rechargeEntrance() {
			this.$http.get(this.$store.state.url+'api/proxy/rechargeEntrance',{params: { rechargeWay:5 }}).then((res) => {
