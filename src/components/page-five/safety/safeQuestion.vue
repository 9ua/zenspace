<template>
  <div class="safeQuestion">
    <div class="safeQuestion-top">
      <router-link to="/safety" tag="i" class="el-icon-arrow-left"></router-link>
      <p>验证密保问题</p>
    </div>
    <div class="safeQuestion-input">
      <div>
        <p>问题一:</p>
        <select v-model="title1" disabled>
          <option v-for="(item,index) in options" :key="index" :value="item.value" disabled>{{item.label}}</option>
        </select>
      </div>
      <div>
        <p>答案:</p>
